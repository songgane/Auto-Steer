...
#
"""Preprocess TrinoDB's Json query plans before fitting them into the tree convolutional neural network"""
import numpy as np
from utils.custom_logging import logger
from inference.preprocessing.preprocessor import QueryPlanPreprocessor

# The following operators can be found in Trino query plans
OUTPUT = 'Output'
AGGREGATE = 'AGGREGATE'
...
# The following attributes can be found in each node of a trino query plan
CHILDREN = 'children'
NODE_TYPE = 'name'
ESTIMATES = 'estimates'
...
# The following attributes can be found in the trino query plan statistics of leaf nodes (e.g. TableScan, ScanFilterProject, etc.)
ROWS = 'outputRowCount'
ROW_SIZE = 'outputSizeInBytes'
CPU_COST = 'cpuCost'
...
def _get_plan_stats(data):
    ...
    def process_estimates(node):
        ...
        if isinstance(estimates, list):
            cpu_estimate = estimates[0][CPU_COST]
            rows_estimate = estimates[0][ROWS]
        else:
...
class TrinoPlanPreprocessor(QueryPlanPreprocessor):
    """"Transforms TrinoDB query plans into a form processable by the TCNNs"""
    ...
    def preprocess(self, plan):
        ...
        if ESTIMATES in plan:
            if len(plan[ESTIMATES]) == 0:
                ...
                #FIXME: scan operator의 경우 estimates가 3개로 구성됨
                # ex. Estimates: {rows: 60175 (3.67MB), cpu: 8.22M, memory: 0B, network: 0B}/{rows: 58074 (3.54MB), cpu: 8.22M, memory: 0B, network: 0B}/{rows: 58074 (3.54MB), cpu: 3.54M, memory: 0B, network: 0B}
                # 첫번째는 스캔 작업의 추정치, 두번째는 필터링 후의 추정치, 세번째는 푀종 프로젝션 단계의 추정치를 가리킴ㅡ먀ㅜ
                plan[ESTIMATES] = plan[ESTIMATES][0]
